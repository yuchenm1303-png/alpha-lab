...
class FactorProvider:
    def fetch_factor_values(self, start_date, end_date, *, symbols=None):
        assert start_date == date(2026, 9, 1)
        assert end_date == date(2026, 9, 2)
        assert symbols == ["000001.SZ", "600519.SH"]
        return [
            ("000001.SZ", "2026-09-01", "is_limit_up", 1.0, "test")
        ]
...
def test_sync_joins_independent_sources_into_repository(tmp_path):
    ...
    assert summary.bar_start_date == date(2026, 8, 18)
    assert summary.bar_end_date == date(2026, 12, 25)
    assert summary.unique_symbols == 2
    assert repo.stats()["bars"] == 2
    assert repo.stats()["popularity_rows"] == 2
    with db.connect() as conn:
        assert conn.execute(
            "SELECT value FROM factor_values WHERE factor_id = 'is_limit_up'"
        ).fetchone()[0] == 1.0
